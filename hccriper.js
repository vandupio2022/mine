...
for (var i=0; i <tablex.length;i++)
{
	...
	//targets
	var target = tablex[i].querySelectorAll("td div.translate-y ul")[0].querySelectorAll("p");
	for(var tr=0; tr<target.length;tr++)
	{
		targets = target[tr].innerText+"\n"+targets;
	}
	...
	result.push(property+"\t"+name+"\t"+state+"\t"+sLang+"\t"+devicex+"\t"+IId+"\t"+targets)
	i++;
}
